...
import com.goldskyer.gmxx.common.vos.WeixinVo;
...
{
	public static void setMenu(WeixinVo weixinVo)
	{
		...
		b1Subs.add(new Button("光小名片", "view", weixinVo.getMingpian()));//"http://v6.rabbitpre.com/m/riqQFrJ"
		...
		b2Subs.add(new Button("名师风采", "view", weixinVo.getMingshi())); //http://v7.rabbitpre.com/m/uEfqyzVRx
		b2Subs.add(new Button("班级社区", "view", host + "/front/class/view.htm")); //http://v7.rabbitpre.com/m/uEfqyzVRx
		b2.setSub_button(b2Subs);
		Button b3 = new Button("智慧光小", "");
		List<Button> b3Subs = new ArrayList<>();
		b3Subs.add(new Button("光小电视台", "view", host + "/front/tv/list.htm?blogType=每周视讯"));
		b3Subs.add(new Button("直播光小", "view", host + "/front/tv/list.htm?blogType=课堂直播"));
		b3Subs.add(new Button("微课中心", "view", host + "/front/tv/list.htm?blogType=语文"));
		b3Subs.add(new Button("学习中心", "view", host + "/front/learnCenter/list.htm?blogType=学生"));
		...
		Button fankuiBtn = new Button("意见反馈", "click", "http://www.baidu.com");
		...
		try
		{
			result = postToUrlReturnMoreMessage(
					" https://api.weixin.qq.com/cgi-bin/menu/create?access_token="
							+ tokenJson.getString("access_token"),
					JSONObject.fromObject(body).toString(), "UTF-8", 20000, 20000);
	...
	public static String postToUrlReturnMoreMessage(String strUrl, String postData, String charSet, int connectTimeout,
			int readTimeout) throws UnsupportedEncodingException
	{
		InputStream is = null;
