	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "//cdn.goldskyer.com/library/upload-img/control/images/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onDelete: function(file, files) {
						...
						console.info("剩下的文件");
						console.info(files);
						if($("input[name='picUrl']").val().indexOf(file.name)>0){
							var strs= new Array();
							var newstr="";
							strs = $("input[name='picUrl']").val().split('|');
							for (i=0;i<strs.length ;i++ ) 
							{ 
								if(strs[i].indexOf(file.name)<=0){
									newstr+=strs[i];
								}
							}
							$("input[name='picUrl']").val(newstr);
						}
					},
					...
					onSuccess: function(file, response) {
						...
						var url=$("input[name='picUrl']").val()+responseObj.data.picUrl;
